...
func defaultZonesGRPCClientOptions() []option.ClientOption {
...
func defaultZonesCallOptions() *ZonesCallOptions {
...
// zonesGRPCClient is a client for interacting with Google Compute Engine API over gRPC transport.
//
// Methods, except Close, may be called concurrently. However, fields must not be modified concurrently with method calls.
type zonesGRPCClient struct {
	...
	// The gRPC API client.
	zonesClient computepb.ZonesClient

	// The x-goog-* metadata to be sent with each request.
	xGoogHeaders []string
}

// NewZonesClient creates a new zones client based on gRPC.
// The returned client must be Closed when it is done being used to clean up its underlying connections.
//
// The Zones API.
func NewZonesClient(ctx context.Context, opts ...option.ClientOption) (*ZonesClient, error) {
	clientOpts := defaultZonesGRPCClientOptions()
	if newZonesClientHook != nil {
		hookOpts, err := newZonesClientHook(ctx, clientHookParams{})
		if err != nil {
			return nil, err
		}
	...
	client := ZonesClient{CallOptions: defaultZonesCallOptions()}

	c := &zonesGRPCClient{
		connPool:    connPool,
		zonesClient: computepb.NewZonesClient(connPool),
...
// Deprecated: Connections are now pooled so this method does not always
// return the same resource.
func (c *zonesGRPCClient) Connection() *grpc.ClientConn {
...
// the `x-goog-api-client` header passed on each request. Intended for
// use by Google-written clients.
func (c *zonesGRPCClient) setGoogleClientInfo(keyval ...string) {
...
// Close closes the connection to the API service. The user should invoke this when
// the client is no longer required.
func (c *zonesGRPCClient) Close() error {
	return c.connPool.Close()
}

func (c *zonesGRPCClient) Get(ctx context.Context, req *computepb.GetZoneRequest, opts ...gax.CallOption) (*computepb.Zone, error) {
	...
	err := gax.Invoke(ctx, func(ctx context.Context, settings gax.CallSettings) error {
		var err error
		resp, err = c.zonesClient.Get(ctx, req, settings.GRPC...)
		return err
	}, opts...)
	if err != nil {
		return nil, err
...
func (c *zonesGRPCClient) List(ctx context.Context, req *computepb.ListZonesRequest, opts ...gax.CallOption) *ZoneIterator {
	...
	it.InternalFetch = func(pageSize int, pageToken string) ([]*computepb.Zone, string, error) {
		...
		err := gax.Invoke(ctx, func(ctx context.Context, settings gax.CallSettings) error {
			var err error
			resp, err = c.zonesClient.List(ctx, req, settings.GRPC...)
			return err
		}, opts...)
		if err != nil {
			return nil, "", err
